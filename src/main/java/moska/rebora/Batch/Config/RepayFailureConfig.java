        ...
        return new ItemProcessor<Payment, Payment>() {
            ...
            public Payment process(Payment payment) throws Exception {
                ...
                if (payment.getPaymentStatus() == PaymentStatus.COMPLETE) {
                    notificationService.createNotificationPayment(notificationSubject, notificationContent, NotificationKind.HISTORY, movie.getMovieName(), user, recruitment, payment);
                ...
                if (payment.getPaymentStatus() == PaymentStatus.FAILURE) {
                    ...
                    PaymentLog paymentLog = PaymentLog.builder()
                            ...
                            .paymentLogContent(payment.getPaymentContent())
                            .build();
                    notificationService.createNotificationPayment(notificationSubject, payment.getPaymentContent(), NotificationKind.HISTORY, movie.getMovieName(), user, recruitment, payment);
                    paymentLogRepository.save(paymentLog);
                }
